    ...
    if request.method == "GET":
        ...
        if request.POST["password1"] == request.POST["password2"]:
            try:
                ...
                messages.error(
                    request, "Username already taken, Please try something else!"
                )
                return render(
                    request, "account/signup.html", {"form": CreateUserForm()}
                )
        else:
            messages.error(
                request, "Passwords did not match, Please try something else!"
            )
            return render(request, "account/signup.html", {"form": CreateUserForm()})
